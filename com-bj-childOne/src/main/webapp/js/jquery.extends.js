...
(function($){
	...
	$.extend({
			...
			this.ajax({
				...
				success : function(data) {
						...
						setTimeout(function(){
							if(options.lock)
								$.loading.close();
							if(data.code && "noRightToOperate" === data.code){
								$.alert("数据填报时间已结束！");
							}else{
								if ("json" === options.dataType && "object" !== typeof data) {
									$.alert("很抱歉,网络繁忙操作失败,请稍后重新操作![status:5000]");
		...
		info : function (content, opts, yes, no) {
		    var hideenFlash = function(){
		    	$.dialogCount = $.dialogCount ? $.dialogCount + 1 : 1;
				//隐藏Flash上传组件
		    	$("#SWFUpload_0").css("visibility", "hidden");
		    },showFlash = function(){
		    	$.dialogCount = $.dialogCount ? $.dialogCount - 1 : 0;
				//显示Flash上传组件
		    	if($.dialogCount == 0)
		    ...
		    if(opts && $.isFunction(opts.close)){
				var close = opts.close;
				opts.close = function(here){
					close.call(this, here);
					showFlash.call();
				};
		    }
		    if(opts && $.isFunction(opts.init)){
		    	var init = opts.init;
				opts.init = function(here){
					init.call(this, here);
					hideenFlash.call();
				};
		    }
			...
			return this.dialog.through(options);
		},
		 ...
		 */
		alert : function (content, callback, opts) {
			var options = $.extend({
		        id: "dialog_alert",
		        icon: "warning",
		        ok: true,
		 ...
		 */
		confirm : function (content, yes, no, opts) {
			var options = $.extend({
				id : "dialog_confirm",
				icon : "question",
				ok : function(here) {
		 ...
		 * @param	{String}	默认值
		 */
		prompt : function (content, yes, value, opts) {
			value = value || "";
		    var input,
		    promptContent = [
			...
			return $.info(promptContent, options);
		},
		 ...
		 */
		tips : function (content, callback, time, opts) {
			var options = $.extend({
				id: "dialog_tips",
		        title: false,
		        cancel: false,
			...
			return $.info("<div style='padding: 0 1em;'>" + content + "</div>", options);
		},
		 ...
		 */
		loading : function (content, timeout, opts) {
			var options = $.extend({
				id: "dialog_loading",
				title: false,
				cancel: false,
			...
			return $.info("", options)
				.content(["<div style='padding: 0 1em;text-align:center;'><img src='",
					this.basePath, "/images/loading.gif' width='32' heigth='32'/><br/>",
					(content || "正在执行操作，请稍后..."), "</div>"].join(""));
		},
		 ...
		 * @param	{funciton}	回调函数
		 */
		msg : function (content, title, callback, opts) {
			var options = $.extend({
				id: "dialog_msg",
		        title: (title || "信息"),
		        close: callback
